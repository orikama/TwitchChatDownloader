...
namespace TwitchChatDownloader
{
    class App
    {
        ...
        // NOTE: Move them to their class
        private Func<TwitchComment.JsonComments.JsonComment, string> CommentFormatter;
        private Func<TwitchComment.JsonComments.JsonComment, bool> CommentFilter;
        ...
        public async Task InitAsync(string settingsPath)
        {
            ...
            string filter = AppSettings.FilterExpression.Length != 0 ? AppSettings.FilterExpression : 
            CommentFormatter = await Utils.CompileLambdaAsync<Func<TwitchComment.JsonComments.JsonComment, string>>(AppSettings.CommentFormat);
            CommentFilter = await Utils.CompileLambdaAsync<Func<TwitchComment.JsonComments.JsonComment, bool>>(AppSettings.CommentFormat);
        }
        ...
        private void WriteComments()
        {
            foreach (var (streamWriter, jsonComments) in _commentsPipe.GetConsumingEnumerable()) {
                foreach (var comment in jsonComments.Comments) {
                    string formattedComment = CommentFormatter(comment);
                    streamWriter.WriteLine(formattedComment);

                    if(CommentFilter(comment) == false) {
